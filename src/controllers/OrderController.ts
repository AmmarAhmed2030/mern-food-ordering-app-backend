...
const createCheckoutSession = async (req: Request, res: Response) => {
  try {
    ...
    if (!session.url) {
      return res.status(500).json({ message: "Error crating stripe session " });
    }
    await newOrder.save();
...
const createLineItems = (
  ...
  // 1-foreach cartItem, get menuitemobject from the restaurant
  //(to get the price )
  // for each cart item, convert it to a stripe line item
  ...
  const lineItems = checkoutSessionRequest.cartItems.map((cartItem) => {
    ...
    if (!menuItem) {
      throw new Error(`Menu iten not found :${cartItem.menuItemId}`);
    }
